...
const register = async (req, res) => {
  try {
    ...
    const user = await User.create({ username, email, password: hashed, bio, skills, artInterests });
    res.status(201).json({
      _id: user._id,
...
const getMe = async (req, res) => {
  try {
    ...
    const artworks = await require('../models/Artwork').find({ artist: user._id }).sort({ createdAt: -1 });
